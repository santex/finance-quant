    ...
    foreach my $sym (@ok) {
      ...
      for my $i (reverse 0..$#{$self->{'result'}->{$sym}->{'nasdaq-guru'}}) {
       ...
       my $q = quotes::get($stocksymbol, $startdate, $enddate, $interval, $agent);
        ...
        if($ff =~ /<b>(.*)ratings<\/b>/){
    #      $self->writeFile($1,sprintf( "ratings/%s.php",$sym ));
        ...
        if($check==1){
        open OUT,">ratings/bottom/$sym.php";
        print OUT $out;
        close OUT;
        print "done: bottom-$sym.php generated.\n";
        }elsif($check==2){
        open OUT,">ratings/top/$sym.php";
        print OUT $out;
        close OUT;
        print "done: top-$sym.php generated.\n";
        
        }elsif($check==0){
        
        open OUT,">ratings/inbetween/$sym.php";
        print OUT $out;
        close OUT;
        print "done: inbetween-$sym.php generated.\n";
...
{package chart;
  ...
  use Data::Dumper;
	...
	sub html {
		my ($stock, $q, $ma, $diff,$extended,$date) = @_;
		my $str = "";
		my $list = "";
...
#						@ext =@ext[0];
    ...
    $xguru .="</ul><h3>check:gurus nasdaq</h3><ul>";
		...
		$xguru .= sprintf("</ul><h3>Quote</h3><ul>");
        foreach(@quote){
        $xguru .= sprintf("<li>%s</li>",$_);
        }
        $xguru .= sprintf("</ul>\n\n");
...
my $meta = '<meta name="Description" content="html5stockbot.com does '.
           'Cluster Analysis and Portfolio correlations'.
           ' automated trading and stocks analysis using '.
           'technology like R, Quantstrat, Quantmod, PDL, GNUPLOT ">'.
           '<meta name="robots" content="all">'.
           '<meta name="keywords" content="quantstrat, blotter,ASSET ALOCATION,'.
           'Backtest,quant,Gnuplot,R,quantmod,finance  ,market, business ,'.
           'stocks ,signals,daily,analysis, news, economy, technical,in-depth stock analysis,'.
           'online stock trading,track stocks,news ,economy ,investment selection, '.
           'live trading stock tips,ibes,Broker">';
		
		
$str = "<html><head>$meta\n<title>\$$stock ,$date ,#$stock</title></head>".
        "<body bgcolor=\"gray\" text=\"white\">".
		"<iframe frameborder='0' src='http://html5stockbot.com/data/header.php?symbol=".$stock.
		"' style='left:0; top:0;position:absolute;border:0;height: 40px; width: 100%'></iframe><br /><br /><br />".
	    "<table style='width: 100%' cellspacing=5 cellpadding=5><tr><td valign='top'>".
	    "<div style='float:left;'>".$xguru."</div>".
	    "</td><td valign='top'>".
	    "<div style='float:right;'>\n";

		$str .= "<img src=\"data:image/png;base64," . plotlog($stock, $q, $ma) . "\">";
		$str .= "<img src=\"data:image/png;base64," . plotdiff($stock, $q, $ma, $diff) . "\"></div>";
		#$str .=  _tbl($stock, $q);
		
		$str .= '<hr /><p><iframe frameborder="0" src="http://docs.google.com/gview?url=http://html5stockbot.com/data/upload/backtest/longtrend_backtest_'.$stock.'.pdf&amp;embedded=true" height="2300px" style="height: 2300px; width:98%;"></iframe>'.
		'<hr /><p><iframe frameborder="0" height="8900px" src="http://docs.google.com/gview?url=http://html5stockbot.com/data/upload/charts/chart_'.$stock.'.pdf&amp;embedded=true" style="height: 7900px;" width=98%;></iframe></p></div></body></html>';
	...
	sub plotlines {
		...
		my $graph = GD::Graph::lines->new(800,380);
